...
// Version: 1.0.5 - Comprehensive BiDi and Blob Fix
const generateId = () => {
...
export default function TasksPage() {
  ...
  const handleExportPDF = async () => {
    ...
    setTimeout(() => {
      ...
      const opt = { margin: 10, filename: `تقرير_الأعمال_${new Date().toLocaleDateString('ar-EG')}.pdf`, image: { type: 'jpeg', quality: 0.98 }, html2canvas: { scale: 2, useCORS: true, letterRendering: true }, jsPDF: { unit: 'mm', format: 'a4', orientation: 'portrait' }, pagebreak: { mode: ['avoid-all', 'css', 'legacy'] } };
  ...
  return (
    <div style={{ background: 'var(--bg)', minHeight: '100vh', padding: '20px', paddingBottom: '100px' }}>
      <div style={{ maxWidth: '800px', margin: '0 auto' }}>
            ...
            {filteredTasks.map(task => (
              <div key={task.id} onClick={() => { setSelected(task); setView('detail'); }} style={{ background: 'var(--surface)', padding: '20px', borderRadius: '16px', border: '1px solid var(--border)', cursor: 'pointer', boxShadow: 'var(--shadow)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                <div style={{ display: 'flex', gap: '15px', alignItems: 'center' }}>
                  ...
                  <div><h3 style={{ fontSize: '15px', fontWeight: 900, color: 'var(--text)', margin: 0, ...bidiStyle }}>{task.title}</h3><p style={{ fontSize: '12px', color: 'var(--text-3)', fontWeight: 700, ...bidiStyle }}>{getClientName(task.client_id)}</p></div>
                </div>
        ...
        {/* DETAIL VIEW WITH UNIVERSAL BIDI FIX */}
        ...
        {view === 'form' && (
          <div style={{ background: 'var(--surface)', padding: '30px', borderRadius: '24px', border: '1px solid var(--border)' }}>
             ...
             <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                ...
                <div style={{ border: '2px dashed var(--border)', borderRadius: '16px', padding: '20px', textAlign: 'center' }}><input type="file" multiple id="fileX" hidden onChange={handleFileUpload} /><label htmlFor="fileX" style={{ cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px' }}>{loadingFile ? <Loader2 size={24} className="animate-spin" /> : <Paperclip size={24} color="var(--blue)" />}<span style={{ fontWeight: 900, fontSize: '13px' }}>إرفاق مرفقات</span></label></div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '30px' }}>
                {filteredTasks.map((task, i) => (
                  <div key={task.id} style={{ border: '1px solid #e2e8f0', borderRadius: '12px', padding: '20px', pageBreakInside: 'avoid' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px' }}><h3 style={{ margin: 0, fontSize: '16px', ...bidiStyle }}>{i + 1}. {task.title}</h3><span style={{ fontSize: '12px', fontWeight: 800, color: '#2563eb' }}>{task.status}</span></div>
                    <div style={{ fontSize: '11px', color: '#64748b', marginBottom: '10px', ...bidiStyle }}>العميل: {getClientName(task.client_id)} {task.plot_number ? `| قسيمة: ${task.plot_number}` : ''}</div>
                    <div style={{ fontSize: '13px', background: '#f8fafc', padding: '15px', borderRadius: '8px', marginBottom: '10px', borderRight: '3px solid #2563eb', color: '#000', ...bidiStyle }}>
                    ...
                    {task.updates?.length > 0 && (
                      <div style={{ marginTop: '10px', padding: '10px', background: '#f1f5f9', borderRadius: '8px' }}>
                         <div style={{ fontSize: '12px', fontWeight: 900, marginBottom: '8px', color: '#475569' }}>📝 سجل المتابعة:</div>
                         {task.updates.map((up, j) => (<div key={j} style={{ fontSize: '11px', marginBottom: '4px', borderBottom: '1px solid #e2e8f0', ...bidiStyle }}><span style={{ fontWeight: 900 }}>{up.user}: </span> {up.text}</div>))}
                      </div>
                    )}
                  </div>
                ))}
